...
myArray[1]= false;
myArray[3][0]=6;
// output.textContent = "The 3rd element in the array is" + String(myArray[2])+ " which is type of"+Typeof(myArray[2])+"!";

// output.textContent = `The 3rd element in the array is ${String(myArray[2])} which is type of"${Typeof(myArray[2])}!`;

// You can also change a particular element
myArray[1]= false;
// console.log(The second element )
...
// output.textContent=`myArray has ${myArray.lenght} element inside of it`;

output.textContent=`myArray has ${myArray[3].lenght} element inside of it`;
...
let orig6 = "Toronto Maple Leaf, Chivago Blackhawk, Detroit Red Wings, Boston Burins, Montreal Canadians, New York Rangers";

let orig6Array= orig6.split(",");
output.textContent= orig6Array[4];

// Output one of the array items
output.textContent= `The last tem in the Array is ${orig6Array.at9(-1)}.`
...
numItems= orig6Array.unshift("Winnipeg Jets");
